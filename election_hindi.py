...
from election_modules import (
    EnhancedParser, Config, processing_logger, 
    transliterate_hindi_to_english, split_name, 
    validate_voter_entry, encode_image_to_base64
)

# Initialize the enhanced parser
parser = EnhancedParser('hindi')

# validate_voter_entry is now imported from election_modules
...
def process_page_parallel(args):
    ...
    page_image.save(image_path, 'JPEG', quality=Config.OCR_IMAGE_QUALITY)
    
    # Process the page with enhanced parser
    voter_entries = parser.parse_page(image_path, page_num, expected_entries=30)
    
    # Post-process entries for Hindi-specific requirements
    processed_entries = []
    for entry in voter_entries:
        # Handle transliteration if needed
        if "name_english" not in entry or not entry["name_english"]:
            if "name_hindi" in entry and entry["name_hindi"]:
                try:
                    entry["name_english"] = transliterate_hindi_to_english(entry["name_hindi"])
                except Exception as e:
                    processing_logger.error(f"Error transliterating name: {e}")
                    entry["name_english"] = ""
            else:
                entry["name_english"] = ""

        # Split name into components
        if "name_english" in entry and entry["name_english"]:
            try:
                name_parts = split_name(entry["name_english"])
                ...
                processing_logger.error(f"Error splitting name: {e}")
                entry["first_name"] = ""
                entry["middle_name"] = ""
                entry["last_name"] = ""
        else:
        ...
        # Validate entry
        if validate_voter_entry(entry, page_num, 'hindi'):
...
def process_electoral_roll(pdf_path, output_csv, start_page=Config.DEFAULT_START_PAGE, end_page=None, check_total=True, max_parallel_workers=Config.DEFAULT_MAX_PARALLEL_WORKERS):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        max_workers = min(max_parallel_workers, len(pages))
    ...
    if all_voter_entries:
        ...
        required_columns = ['serial_no', 'name_hindi', 'name_english', 'father_husband_hindi', 'age', 'gender', 'eid', 'first_name', 'middle_name', 'last_name']
        ...
        df = df.rename(columns={
            ...
            'father_husband_hindi': 'Father/Husband Name (Hindi)',
            'age': 'Age',
            'gender': 'Gender',
            'eid': 'EID',
        ...
        pd.DataFrame(columns=['Serial No', 'Name (Hindi)', 'Name (English)', 'Father/Husband Name (Hindi)', 'Age', 'Gender', 'EID', 'First Name', 'Middle Name', 'Last Name']).to_csv(output_csv, index=False)
...
if __name__ == "__main__":
    ...
    pdf_path = "/home/lt-340/Desktop/Election/2025-EROLLGEN-U05-2-DraftRoll-Revision1-HIN-1-WI.pdf"
    output_csv = "/home/lt-340/Desktop/Election/voter_data_gemini_U05_hindi.csv"
    ...
    processing_logger.info(f"PDF has {total_pdf_pages} total pages")
    processing_logger.info(f"Processing pages {start_page} to {end_page} with {max_workers} parallel workers")
    processing_logger.info(f"The last page (page {total_pdf_pages}) will be used only for verification of total voters")
    ...
    print("\nUsage:")
    print("  python election_hindi_updated.py [start_page] [end_page] [max_workers]")
    print("\nExamples:")
    print("  python election_hindi_updated.py                # Process all pages with default workers")
    print("  python election_hindi_updated.py 3 7           # Process pages 3-7 with default workers")
    print("  python election_hindi_updated.py 3 7 5         # Process pages 3-7 with 5 workers")
    print("  python election_hindi_updated.py 8 17 2        # Process pages 8-17 with 2 workers")
    print(f"  python election_hindi_updated.py 3 {total_pdf_pages-1} 4  # Process all pages from 3 to {total_pdf_pages-1} with 4 workers")